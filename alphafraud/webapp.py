...
def create_app() -> Flask:
    ...
    def statistics():
        ...
        dep = f"{(e.get('dep_min') or '—')[:10]}→{(e.get('dep_max') or '—')[:10]}"
        rel = f"{(e.get('rel_min') or '—')[:10]}→{(e.get('rel_max') or '—')[:10]}"
        load = "/".join(f"{x:.2f}" for x in sysm["load"]) if sysm.get("load") else "—"
        mem = f"{_fmt_bytes(sysm.get('mem_used'))}/{_fmt_bytes(sysm.get('mem_total'))}" if sysm.get("mem_total") else "—"
        disk = f"{_fmt_bytes(sysm.get('disk_used'))}/{_fmt_bytes(sysm.get('disk_total'))}" if sysm.get("disk_total") else "—"
        panels = [
            {"icon": "🔬", "title": "Catch & quality", "rows": [
                ("Analysed", _fmt_int(n), url_for("index")),
                ("Compared (tier-2)", _fmt_int(e.get("compared")), None),
                ("Screened (tier-1)", _fmt_int(e.get("screened")), None),
                ("Confidently wrong", f"{_fmt_int(e.get('cw'))} · {rate(e.get('cw'))}", url_for("leaderboard")),
                ("Novel", f"{_fmt_int(e.get('novel'))} · {rate(e.get('novel'))}", None),
                ("Novel+wrong", f"{_fmt_int(e.get('novel_cw'))} · {rate(e.get('novel_cw'))}", None),
                ("Proteins (UniProt)", _fmt_int(e.get("proteins")), None),
                ("Mean TM", f"{e.get('avg_tm') or 0:.3f}", None),
                ("Mean lDDT", f"{e.get('avg_lddt') or 0:.3f}", None),
                ("Mean pLDDT", f"{e.get('avg_plddt') or 0:.1f}", None),
                ("Skipped", _fmt_int(s["skipped"]), None),
                ("Errors", _fmt_int(s["errors"]), None),
            ]},
            {"icon": "📅", "title": "Coverage & schedule", "rows": [
                ("Deposit range", dep, None),
                ("Release range", rel, None),
                ("Release weeks", _fmt_int(s["release_weeks"]), url_for("archive")),
                ("Backfill months", _fmt_int(rn.get("backfill")), None),
                ("Weekly runs", _fmt_int(rn.get("weekly")), None),
                ("AF cutoff", config.AF_TRAINING_CUTOFF.isoformat(), None),
                ("Last update", _fmt_ts(s["last_run"]["finished_at"]) if s["last_run"] else "—", None),
                ("Next release", next_up, None),
            ]},
            {"icon": "⚙️", "title": "Runs & compute", "rows": [
                ("Total runs", _fmt_int(sum(rn.values()) if rn else 0), None),
                ("Weekly/backfill/calc", f"{rn.get('weekly', 0)}/{rn.get('backfill', 0)}/{rn.get('calculate', 0)}", None),
                ("Latest weekly", s["latest_weekly"]["label"] if s["latest_weekly"] else "—", None),
                ("Ribbons", _fmt_int(sysm.get("ribbon_count")), None),
                ("On-demand calcs", _fmt_int(s["calculated"]), url_for("calculate_page")),
                ("Computing now", _fmt_int(s["computing"]), None),
            ]},
            {"icon": "🖥️", "title": "Server", "rows": [
                ("Version", f"v{sysm.get('app')}", None),
                ("Python", sysm.get("python", "—"), None),
                ("CPU cores", _fmt_int(sysm.get("cpu")), None),
                ("Load (1/5/15)", load, None),
                ("Memory (used/total)", mem, None),
                ("Worker RSS", _fmt_bytes(sysm.get("rss")), None),
                ("Uptime", _fmt_dur(sysm.get("uptime")), None),
            ]},
            {"icon": "💾", "title": "Storage & database", "rows": [
                ("Database (+WAL)", _fmt_bytes(db.db_size_bytes()), None),
                ("Ribbons", f"{_fmt_int(sysm.get('ribbon_count'))} · {_fmt_bytes(sysm.get('ribbon_bytes'))}", None),
                ("Structure cache", _fmt_bytes(sysm.get("struct_bytes")), None),
                ("Data total", _fmt_bytes(sysm.get("data_bytes")), None),
                ("Disk (used/total)", disk, None),
                ("Disk free", _fmt_bytes(sysm.get("disk_free")), None),
                ("entities", _fmt_int(s["table_counts"].get("entities")), None),
                ("entity_blobs", _fmt_int(s["table_counts"].get("entity_blobs")), None),
                ("entity_annotations", _fmt_int(s["table_counts"].get("entity_annotations")), None),
                ("weekly_examples", _fmt_int(s["table_counts"].get("weekly_examples")), None),
                ("runs/visits", f"{_fmt_int(s['table_counts'].get('runs'))}/{_fmt_int(s['table_counts'].get('visits'))}", None),
            ]},
            {"icon": "🔌", "title": "Data sources & APIs", "rows": [
                ("RCSB Search", "search.rcsb.org", "https://search.rcsb.org"),
                ("RCSB GraphQL", "data.rcsb.org", "https://data.rcsb.org/graphql/index.html"),
                ("RCSB files", "files.rcsb.org", "https://www.rcsb.org"),
                ("AlphaFold DB", "alphafold.ebi.ac.uk", "https://alphafold.ebi.ac.uk"),
                ("Calculate index", _fmt_int(s["table_counts"].get("calculate_index")), url_for("calculate_page")),
                ("AF checks cached", _fmt_int(s["table_counts"].get("af_uniprot_cache")), None),
                ("Source code", "bellcheddar/AlphaFraud", "https://github.com/bellcheddar/AlphaFraud"),
            ]},
            {"icon": "👥", "title": "Traffic & coverage", "rows": [
                ("Visitors", _fmt_int(vs.get("unique")), None),
                ("Page views", _fmt_int(vs.get("hits")), None),
                ("Today", _fmt_int(vs.get("today")), None),
                ("Total processed", _fmt_int(total), None),
                ("Coverage", f"{100 * n / total:.1f}%" if total else "—", None),
            ]},
        ]
